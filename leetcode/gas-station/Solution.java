class Solution {
    ...
    2 points:
    1. if A cant reach B, every station between A and B cant reach B: gas A - cost A + gas A2 - cost A2 + gas B < cost B
    2. if total gas < total cost, no solution
    
    */
    public int canCompleteCircuit(int[] gas, int[] cost) {
        int start = gas.length - 1;
        int end = 0;
        int remaining = gas[start] - cost[start];
        while (start != end) {
            if (remaining >= 0) {
